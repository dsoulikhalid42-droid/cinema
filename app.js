    ...
    if (!trendingData || !recommendedData || recommendedData.length === 0) {
        appContainer.innerHTML = `
            <div style="text-align:center; padding:80px 5%; line-height:1.6;">
                <h3 style="color: #ff4444;">⚠️ Data Loading Failed</h3>
                <p>Please make sure your TMDB_API_KEY is pasted inside config.js correctly.</p>
            </div>`;
        return;
    }
...
async function renderDetailsView(id, type) {
    appContainer.innerHTML = '<div style="text-align:center; padding:100px 0;"><i class="fas fa-circle-notch fa-spin fa-2x teal-glow-text"></i></div>';
